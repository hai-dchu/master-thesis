...
from util.misc import (  # , interpolate, inverse_sigmoid
    NestedTensor,
    nested_tensor_from_tensor_list,
)

from .backbone_v1 import build_backbone
    ...
    def __init__(
        ...
        if masked_attn:
            self.attention_mask = torch.ones(
                (num_queries, num_queries), dtype=torch.bool
            )
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices):
        ...
        if "pred_room_logits" in outputs:
            ...
            room_target_classes_o = torch.cat(
                [t["room_labels"][J] for t, (_, J) in zip(targets, indices)]
            )
            room_target_classes = torch.full(
                room_src_logits.shape[:2],
                self.semantic_classes - 1,
                dtype=torch.int64,
                device=room_src_logits.device,
            )
            room_target_classes[idx] = room_target_classes_o
            loss_ce_room = F.cross_entropy(
                room_src_logits.transpose(1, 2), room_target_classes
            )
    ...
    def loss_cardinality(self, outputs, targets, indices):
        ...
        tgt_lengths = (
            torch.as_tensor([sum(v["lengths"]) for v in targets], device=device) / 2
        )
    ...
    def loss_polys(self, outputs, targets, indices):
        ...
        target_len = torch.cat(
            [t["lengths"][i] for t, (_, i) in zip(targets, indices)], dim=0
        )
        ...
        if self.semantic_classes == -1:
            loss_raster_mask = self.raster_loss(
                src_polys.flatten(1, 2), target_polys, target_len
            )
...
class DinoImageFeature(nn.Module):
    def __init__(
        self, backbone, head, n_last_layers: int = 1, img_size: tuple = (256, 256)
    ):
        super().__init__()
        self.backbone = backbone
        self.head = head
...
def build(args, train=True):
    ...
    embed_dim = [dinov3.embed_dim] * args.dinov3_n_last_layers
    linear_head = LinearHead(embed_dim, n_output_channels=1, dropout=0.1).to(
        args.device
    )
    dinov3_feature_extractor = DinoImageFeature(
        backbone=dinov3,
