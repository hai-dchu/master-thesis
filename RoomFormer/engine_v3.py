    ...
    for batched_inputs in metric_logger.log_every(data_loader, 10, header):
        ...
        for i in range(bs):
            ...
            if dataset_name == "stru3d":
                ...
                quant_result_dict_scene = evaluator.evaluate_scene(
                    room_polys=room_polys, gt_polys=gt_polys
                )
            ...
            if semantic_rich:
                metric_logger.update(
                    room_sem_prec=quant_result_dict_scene["room_sem_prec"]
                )
                metric_logger.update(
                    room_sem_rec=quant_result_dict_scene["room_sem_rec"]
                )
                metric_logger.update(
                    window_door_prec=quant_result_dict_scene["window_door_prec"]
                )
                metric_logger.update(
                    window_door_rec=quant_result_dict_scene["window_door_rec"]
                )
        ...
        if plot_gt:
            for i, gt_inst in enumerate(gt_instances):
                if not semantic_rich:
                    ...
                    gt_polys = []
                    density_map = np.transpose(
                        (samples[i] * 255).cpu().numpy(), [1, 2, 0]
                    )
                    ...
                    gt_floorplan_map = plot_floorplan_with_regions(
                        gt_room_polys, scale=1000
                    )
                    cv2.imwrite(
                        os.path.join(output_dir, f"{scene_ids[i]}_gt.png"),
                        gt_floorplan_map,
                    )
                else:
                    ...
                    for j, poly in enumerate(gt_inst.gt_masks.polygons):
                        ...
                        gt_sem_rich.append(
                            [corners, gt_inst.gt_classes.cpu().numpy()[j]]
                        )

                    gt_sem_rich_path = os.path.join(
                        output_dir, f"{scene_ids[i]}_sem_rich_gt.png"
                    )
                    plot_semantic_rich_floorplan(
                        gt_sem_rich, gt_sem_rich_path, prec=1, rec=1
                    )
        ...
        for i in range(pred_logits.shape[0]):
            ...
            if plot_pred:
                if semantic_rich:
                    ...
                    pred_sem_rich_path = os.path.join(
                        output_dir, f"{scene_ids[i]}_sem_rich_pred.png"
                    )
                    plot_semantic_rich_floorplan(
                        pred_sem_rich,
                        pred_sem_rich_path,
                    ...
                    cv2.imwrite(
                        os.path.join(output_dir, f"{scene_ids[i]}_pred_floorplan.png"),
                        floorplan_map,
                    )
            ...
            if plot_density:
                ...
                cv2.imwrite(
                    os.path.join(output_dir, f"{scene_ids[i]}_pred_room_map.png"),
                    pred_room_map,
                )
